  ...
  if installed_by_installer == 'y':
    if input('Is it installed as a service?(y/n)\n') == 'y':
      ...
      if current_platform == 'Linux':
        ...
        jellyfin_path = os.environ["APPDATA"] + 'jellyfin-rpc\\' + 'jellyfin-rpc.exe'
  else:
    jellyfin_path = input('Where is your jellyfin rpc located?\n')
    if "\"" in jellyfin_path:
      jellyfin_path = jellyfin_path.replace("\"", "")
else:
...
config_dict['token'] = input('Please input your discord token [https://github.com/Pr0chy/jellyfin-rpc-discord-bridge/wiki/How-do-i-get-the-discord-token%3F]\n')
